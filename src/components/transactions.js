...
const Transactions = () => {
  ...
  const currentPageData = transactions
    ...
    .map((transaction, index) => {
      return (
        <div key={index} className="flex  border-b items-center justify-center text-center text-[10px] md:text-xs  rounded-md">
          <div className="w-1/3 cursor-default p-4 items-center justify-center truncate">{transaction.returnValues["1"]}</div>
          <a
            href={`${window.location.protocol}//${window.location.host}/t/${toHex(transaction.returnValues.linkId)}`}
            className="w-1/3 p-4 items-center justify-center truncate"
            target="_blank"
            rel="noreferrer"
          >
            {`${window.location.host}/t/${toHex(transaction.returnValues.linkId)}`}
          </a>
  ...
  async function getTransactions() {
    let web3 = new Web3("https://polygon-mumbai.g.alchemy.com/v2/aWibDV94uYYQgHqtHlwXrOVow9kWfbCJ");
